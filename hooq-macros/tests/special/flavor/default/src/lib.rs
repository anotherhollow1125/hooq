...
mod tests {
    ...
    fn test_special_flavor_default() {
        ...
        let pre_expanded = std::fs::read_to_string("tests/tests_inner/default.expanded.rs").ok();

        let output = String::from_utf8_lossy(
            &std::process::Command::new("cargo")
                .args(["expand", "--test", "mod", "tests_inner::default"])
                .output()
                .unwrap()
                .stdout,
        )
            ...
            && pre_expanded != output
        {
            ...
            std::fs::write("tests/tests_inner/default.expanded.rs", output).unwrap();
        }
