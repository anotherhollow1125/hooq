...
- [x] 「`Ok(...)` がtail exprに来る時はフックしない」などを表す `tail_expr_idents` のブラックリスト版を作る
  - `ignore_tail_expr_idents` とかでいいか
- [x] `toml_load!()` マクロを廃止し、ファイルパスは各マクロ呼び出しで指定するようにする。キャッシュ機構のみ設ける
- [x] `#[hooq::method = flavor_name]` での上書きにする
- [ ] README 整形
  - [x] docs.rs への導線設置
  - [x] サブクレートのURL修正忘れていたので直す
    - [x] ~~こっちは crates.io のURLを記載したい~~
      - 結局 GitHub のものにした
  - [ ] とりあえずWIPを埋めたい
...
- [x] `$expr` -> `$expr_str` へのリネーム
  - 検討中を参照のこと
- [x] `$parent_method` でメソッド上書き時に元のメソッドも活用できるようにする
- [x] `$expr` の出力する情報が多いので、出力を控えたものも用意する
- [x] `fn_info` におけるクロージャの出力について、どの関数の中のクロージャであるか明記できるようにする
- [ ] 残りのフレーバーの用意
  - preset flavors
...
- [x] dependabot の挙動調査および必要なら修正
- [ ] 軽量化のため、 syn の feature を見直す
- [x] cargo プロジェクトではないrustcでコンパイルする場合でも `toml_load!()` でカレントディレクトリの `hooq.toml` を読み込むことの確認
- [x] このTODOをGitHub issuesで管理するように切り替える
- [ ] `hooq_inner` を晒し、サードパーティ `xxx` が追加のflavor等を実装したラッパー `xxx::hooq` を提供できるようにする
- [x] メソッドじゃない関数などもフック可能にする (検討中の★★参照のこと)
...
- ~~`$get_or_default(xxx, Expr)`: もし存在しない binding の場合、 `Expr` を適用~~
  ...
  - `#[hooq::method(...)]` などと同時に変数自体設定できるわけだから不要と気づいた
...
- ~~`hooq.toml` はプロジェクトルートから現在対象としているファイルパス上のどこかにあればそれを読み込むようにする~~
  ...
  - 安定しないのでクレートルート以外の `hooq.toml` はそもそも許さないこととした
...
- ★★ `hooq::method` に格納されている構文がドットで始まるか否かでフックのモードを変える機能
  ...
  - 末尾が `!` の場合かつ `expr?` にフックするとき、末尾の `?` を削除する...？
- `flavor_create! {}` マクロ
  - 現状tomlファイルを作らなくてもflavorを作れるので、だったらいっそそのまま書けるものも用意したら？と思った

MEMO: 2025/10/18 をもって、タスク管理をGitHub Projectsの方に移動した
