...
- [ ] tomlで設定可能にする機能
  ...
  - [x] `hook(path::to::Trait)` 記法 ( 旧 `custom` 機能 ) をほかのプリセットでも可能とする
    - そもそもこの記法やめる？
      ...
      - `#[hooq(trait_use(...), flavor = "hook")]` 等はあり
        - [ ] flavor 実装後、上記のように書けるようにする (テストで確認)
  - **`trait_use` を消す** ...？
  ...
  - [ ] 関係するテストを実装する
    ...
    - [ ] `hook` 関連のテスト
    - [ ] エラー時の flavor サジェスト
    - [ ] `#[hooq(trait_use(...), flavor = "hook")]` が書けるようになっていることのテスト

# 急がないtodo

急がないtodoの中でリリース前にやっておきたいものに @ を付与

- [ ] @ `$expr` -> `$expr_str` へのリネーム
  - 検討中を参照のこと
...
- [ ] `#[hooq::method = "rel_file/to/hook.rs"]`
  ...
  - `#[hooq::method = flavor_name]` での上書きはどうか
